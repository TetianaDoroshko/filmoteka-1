...
export async function showWatchedMovies(page) {
  ...
  if (!movieSetId || movieSetId.length === 0) {
    ...
    gallery.innerHTML = `<p class='gallery__info'>${textOne}</p><p class='gallery__info'>${textTwo}</p><div class="cat-gallery"></div>`;
    hideLoader();
  } else {
    showLoader();
...
export async function showQueueOfMovies(page) {
  ...
  if (!movieSetId || movieSetId.length === 0) {
    ...
    gallery.innerHTML = `<p class='gallery__info'>${textOne}</p><p class='gallery__info'>${textTwo}</p></p><div class="cat-gallery"></div>`;
    hideLoader();
  } else {
    showLoader();
    arrayOfPromises = await Promise.all(createPromises(movieSetId));
